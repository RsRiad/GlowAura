...
export default function Cart() {
    ...
    return cartArray.length > 0 ? (
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-left border-separate border-spacing-y-3">
                                <thead className="animate-soft-reveal">
                                    <tr className="max-sm:text-sm text-slate-400 font-bold uppercase tracking-widest text-[9px]">
                                        <th className="pb-4 px-4">Product</th>
                                        <th className="pb-4 text-center">Quantity</th>
                                        <th className="pb-4 text-center">Subtotal</th>
                                        <th className="pb-4 text-center max-md:hidden">Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {cartArray.map((item, index) => (
                                        <tr key={index} className="group">
                                            <td className="py-4 px-4 bg-rose-50/20 rounded-l-2xl border-y border-l border-rose-100/20">
                                                <div className="flex gap-6 items-center">
                                                    ...
                                                    <div className="space-y-1">
                                                        <p className="font-bold text-slate-900 text-base leading-tight tracking-tight">{item.name}</p>
                                                        <p className="text-[9px] uppercase tracking-widest font-bold text-rose-400">{item.category}</p>
                                                        <p className="text-xs font-medium text-slate-400">{currency}{item.price}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td className="py-4 text-center bg-rose-50/20 border-y border-rose-100/20 font-bold text-slate-900 text-base tracking-tight">
        ...
        <section className="min-h-[70vh] mx-6 flex flex-col items-center justify-center text-center animate-soft-reveal">
            ...
            <h1 className="text-4xl md:text-5xl font-bold text-slate-900 tracking-tighter mb-4">Your Cart is Empty</h1>
            <p className="text-slate-500 font-medium text-base mb-8">Start adding some premium finds to your collection.</p>
